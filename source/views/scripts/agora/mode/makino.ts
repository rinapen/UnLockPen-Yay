...
export default async function handleMakinoMode(bot_id, rtmChannel: RtmChannel, rtcClient: IAgoraRTCClient) {
    await playTrack("/assets/audio/makino/first.m4a", true, 1000, rtcClient);

    const emotes = ["ま", "き", "の", "様", "最", "強"];
    // const text = "まきの様最強ww"
    const text = "釈迦釈迦チキンとかいう障害者がボコられるw"
