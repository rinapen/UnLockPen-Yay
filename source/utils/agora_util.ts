  ...
  for (const bot of bots) {
    ...
    if (result && !("success" in result)) {
      ...
      try {
        ...
        const saveResult = await AgoraCacheModel.findOneAndUpdate(
          { conference_call_id },
          {
            $addToSet: { 
              bots: {
        ...
        console.log(`ボット ${bot.user_id} の情報をキャッシュに保存しました`);
      } catch (saveError) {
        console.error(`キャッシュ保存エラー:`, saveError);
        // 保存に失敗しても通話参加は継続
      }
    ...
    if (cached && cached.bots && cached.bots.length > 0) {
      console.log(setColor(colors.green, `♻️ キャッシュから ${cached.bots.length} 個のボット情報を発見しました。 [${bot.user_id}]`, 1));
      
      // 最初のボット情報を使用（複数ある場合は選択ロジックを追加可能）
      const cachedBotInfo = cached.bots[0];
      console.log(setColor(colors.green, `♻️ キャッシュされたボット ${cachedBotInfo.bot_user_id} の情報を再利用します。 [${bot.user_id}]`, 1));
      
      results.push({ success: true, bot, agoraInfo: cachedBotInfo.agoraInfo });
      continue;
    }
      ...
      for (const newBot of availableBots) {
        ...
        if (retry && !("success" in retry)) {
          ...
          try {
            ...
            console.log(`再試行保存結果:`, saveResult);
            console.log(`ボット ${newBot.user_id} の情報をキャッシュに保存しました`);
          } catch (saveError) {
            console.error(`再試行キャッシュ保存エラー:`, saveError);
            // 保存に失敗しても通話参加は継続
          }
